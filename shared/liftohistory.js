...
/** Groups consecutive identical sets back into `3x8 100kg @8` notation. */
export function formatSetGroups(sets) {
  ...
  return chunks
    .map(({ count, set }) => {
      let out = `${count}x${set.reps}`;
      if (set.repsLeft !== null && set.repsLeft !== undefined) out += `|${set.repsLeft}`;
      if (set.isAmrap) out += '+';
      const weight = formatWeight(set.weight, set.unit);
      if (weight !== null) out += ` ${weight}`;
      if (set.rpe !== null && set.rpe !== undefined) out += ` @${set.rpe}`;
      return out;
    })
    .join(', ');
}
 ...
 */
export function serializeLiftohistoryRecord({
  date = new Date(),
  programName = null,
  dayName = null,
  week = null,
  ...
  if (programName) header.push(`program: "${programName}"`);
  if (dayName) header.push(`dayName: "${dayName}"`);
  if (week !== null && week !== undefined) header.push(`week: ${week}`);
  if (dayInWeek !== null && dayInWeek !== undefined) header.push(`dayInWeek: ${dayInWeek}`);
  if (durationSeconds !== null && durationSeconds !== undefined) {
    header.push(`duration: ${Math.max(0, Math.round(durationSeconds))}s`);
  }
  header.push('exercises: {');
