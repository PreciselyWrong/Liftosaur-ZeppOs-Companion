...
function adoptAccountSettings(payload) {
  ...
  applyDisplayHold();
  if (autoPrepareChanged && screen === SCREEN.SESSION && !isNotesModalOpen) renderUI();
}
...
function ensureModalControls(totalPages) {
  ...
  setModalControl(
    modalControls.close,
    {
      x: px(hasPaging ? INFO_NAV.close.x : INFO_NAV.singleClose.x),
      y: px(348),
      w: px(hasPaging ? INFO_NAV.close.w : INFO_NAV.singleClose.w),
    ...
    .then((res) => {
      ...
      return send(MESSAGE_TYPES.GET_SETTINGS)
        ...
        .then((currentRes) => {
          ...
          return send(MESSAGE_TYPES.GET_WORKOUT_NEXT).then((nextRes) => {
...
function handleStartWorkout() {
  if (session.view().totalExercises === 0) return;
  persistAndRender(() => session.startWorkout());
  if (directSync.mode === 'DIRECT') {
    ensureDirectWorkoutStarted().catch((err) => {
      console.log('[liftosaur] start direct sync failed:', err?.message || String(err));
    });
  }
}
...
function handleNextSet() {
  ...
  if (reason) { openTextModal('Phone required', reason); return; }
  if (session.view().timedSet) session.startTimedSet();
  else session.nextSet();
  dayPlan = workoutController.plan();
  updateControllerStatus();
}
...
function renderHomeScreen() {
  let start = null;
  if (serviceMode !== 'DEMO' && defaultWorkoutPlan) {
...
function renderDirectConflictScreen() {
  renderTitle('Sync needs attention', THEME.orange);
  ...
  addWidget(widget.TEXT, {
    ...
    align_v: align.CENTER_V,
    text_style: text_style.WRAP,
    text: directSync.remoteMissing
      ? 'The shared workout is no longer active. Return home or check the phone again.'
      : 'Continue the phone workout here, or resolve it on your phone and retry.',
  });
  ...
  addWidget(widget.BUTTON, {
    ...
    normal_color: THEME.primary,
    press_color: THEME.primaryDeep,
    text: directSync.remoteMissing ? 'Return home' : 'Use phone workout',
    text_size: font('button'),
    click_func: directSync.remoteMissing ? returnAfterDiscard : adoptCurrentWorkout,
  });
...
function renderTopBar(view, onBack) {
  ...
  addLiveLabel('hr', {
    ...
    w: px(120),
    h: px(40),
    color: syncWarning ? THEME.orange : heartRateColor(liveHr),
    text_size: font('button'),
    align_h: align.CENTER_H,
    align_v: align.CENTER_V,
    text_style: text_style.NONE,
    text: syncWarning ? truncate(syncWarning, 16) : formatHeartRate(liveHr),
  });
}
...
function renderPreparedTopBar(view, onOverview, onRest) {
  ...
  addLiveButton('elapsed', {
    ...
    color: view.isWorkoutPaused ? THEME.yellow : THEME.primaryLight,
    text_size: font('micro'), text: elapsedLabel(view), click_func: openWorkoutTimerControls,
  });
  ...
  addLiveLabel('hr', {
    ...
    color: syncWarning ? THEME.orange : heartRateColor(liveHr), text_size: font('micro'),
    text: syncWarning ? 'Sync!' : formatHeartRate(liveHr),
  });
}

function renderChangeUnderline(visible, x, y, width) {
  if (!visible) return;
  addWidget(widget.FILL_RECT, {
    x: px(x), y: px(y), w: px(width), h: px(2), radius: px(1), color: THEME.primary,
  });
}
...
function renderActiveSetScreen(view) {
  ...
  renderChangeUnderline(isResting && pending?.changes.exercise, headerX + Math.max(8, (headerWidth - 64) / 2), 119, 64);
  ...
  renderChangeUnderline(isResting && pending?.changes.weight, 215, controls.rows[0].y + controls.rowHeight - 30, 50);
  ...
  renderChangeUnderline(isResting && pending?.changes.reps, 215, controls.rows[1].y + controls.rowHeight - 30, 50);
  ...
  if (controls.showRpe) {
    ...
    renderChangeUnderline(isResting && pending?.changes.rpe, 215, controls.rows[2].y + controls.rowHeight - 30, 50);
  }
...
function handleExerciseImageChange(_imageUrl, status) {
  ...
  if ((status === 'ready' || status === 'unavailable') && (session.view().state === SESSION_STATES.READY || isOverviewOpen)) {
    renderUI();
    return;
  }
...
function renderScreen() {
  ...
  if (directSync.mode === 'DIRECT' && directSync.conflict) {
    return renderDirectConflictScreen();
  }
...
function tick() {
  ...
  if (view.rest) {
    ...
    patched = updateLiveWidget('restBezel', { color: restStatusColor(view.rest) }) && patched;
  }
