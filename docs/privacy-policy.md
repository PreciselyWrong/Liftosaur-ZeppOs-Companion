...
| Optional watch diagnostics for Lifto Companion and Lifto Workout (up to 12 events each from the current and previous runs: timestamps, product, app/firmware/OS/API versions, device source and screen geometry, screen/session state, widget count, modal/overview/preparation/image-setting flags, safe action names, lifecycle/drawing phases, JavaScript error class/phase, and available memory measurements) | Each product's local storage on the watch and settings storage on your phone after reconnection | From the watch to your paired phone only; never to Liftosaur | Helps identify recent app actions and memory conditions when system logs cannot be exported |
