...
This 0.5.12 beta reuses the 32 most recently used converted pictures across phone Side Service restarts; a missing file is rebuilt. Long-term file retention depends on Zepp and remains under physical validation. Reopen Info to retry an interrupted download.
...
- **Lifto Workout Extension**: The extension runs inside native Zepp Strength Training and Free Training workouts instead of the app list. Adding the data page is device-specific:
...
Free Training support is included in 0.5.7 but remains UNKNOWN on physical hardware. Active 2 testers should look for Lifto under **Workout > Free Training > Settings > More > Data Page > Add Page** and report the exact result.
...
To capture a Lifto restart, enable **Record watch diagnostics** in **Zepp > Profile > your watch > Developer Mode > the affected Lifto app > Settings**, open that app on the watch, and use it normally. For Lifto Workout, open its data page inside a Zepp workout. After a restart, reopen the same app, return to its phone settings, open **Watch diagnostics**, and copy the report. Each product keeps a separate opt-in report with the latest 12 events from the current run and previous run, including safe action names and drawing phases. Available firmware, app, and display metadata are captured automatically. Supported devices may also provide app and system memory samples. The report contains no workout content and cannot include a native crash stack; its last event is a clue, not a confirmed cause. Switching recording off clears the retained report after reconnection.
